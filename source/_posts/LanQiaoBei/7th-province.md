...
public class Main {
	...
	// 判断相邻格子的数字是否连续
	private static boolean isContinuous(int num, int x, int y) {
		// 上方
...
public class Main {
	...
	private static int[] isChosen = new int[12];
	...
	public static void dfs(boolean isVisit[], int i) {
		isVisit[i] = true;
...
分析：最聪明做法：在保证持有奇数的球情况下，尽量取最大的偶数球。

```java 缺少样例
import java.util.Scanner;
public class Main {
	private static int[] ball = new int[3];
	private static int[] init = new int[5];
	...
	private static int afterPick(int self) {
		int pick = -1;
		// 奇 + 奇 = 偶
		// 偶 + 偶 = 偶
		// 持有奇数个球
