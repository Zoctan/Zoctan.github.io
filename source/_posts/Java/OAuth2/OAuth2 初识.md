...
2. HTTP Service：HTTP 服务提供商，本文中简称"服务提供商"，即例子中的 Google。
...
```bash
GET /authorize?response_type=code&client_id=abc123&client_secret=abc&state=test&scope=userinfo&redirect_uri=http%3a%2f%2fbank.com%2findex HTTP/1.1
Host: demo.com
```
...
```bash
HTTP/1.1 302 Found
Location: http://bank.com/index?code=1234-abcd&state=test
```
...
```bash
HTTP/1.1 302 Found
Location: https://bank.com/index?usercancel=1&state=test
```
...
grant_type=authorization_code&code=1234-abcd&redirect_uri=http%3a%2f%2fbank.com%2findex HTTP/1.1
```
...
```bash
GET /authorize?response_type=code&client_id=abc123&state=test&redirect_uri=http%3a%2f%2fbank.com%2findex HTTP/1.1
Host: demo.com
```
...
```bash
HTTP/1.1 302 Found
Location: http://bank.com/index?access_token=2Young2Simple&state=test&expires_in=3600&token_type=bearer&scope=select
```
