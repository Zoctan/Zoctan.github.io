...
流程：
1. 先创建 Maven 主工程。
2. 然后创建两个 Module，一个作为 erver，一个作为 client。（都是勾选 Cloud Discovery 中的 Eureka Server）
3. 在导入完依赖后就可以开始了。
...
同样，添加一个服务消费的 Module，命名为 consumer_ribbon，不仅要添加 Cloud Discovery 中的 Eureka Server，还要添加 Cloud Routing 中的 Ribbon。
...
添加新的 Module，命名为 consumer-feign，添加依赖：
