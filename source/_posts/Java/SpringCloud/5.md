...
之前我对上面的 config-server 依赖写上了版本号，结果一直无法访问到配置内容，这里直接这样添加就行了，会根据 spring boot 的版本选择版本的。
...
然后在项目下建立 config 文件夹，分别添加不同的配置文件，用于给客户端 config-client 进行配置（后面创建的客户端名 spring.application.name=config-client）：
...
config-client 就是 application，dev | test | prod 等就是 profile，label 是分支的名称。
...
```bash
$ curl http://localhost:20000/config-client/dev/
{
    "name": "config-client",
    "profiles": [
        "dev"
    ...
    "propertySources": [
        {
            "name": "https://github.com/Zoctan/spring-cloud-demo.git/config/config-client-dev.properties",
            "source": {
                "from": "git-dev-1.0"
            }
        },
        {
            "name": "https://github.com/Zoctan/spring-cloud-demo.git/config/config-client.properties",
            "source": {
                "from": "git-default-1.0"
            }
        }
...
新增一个 Module，命名为 config_client，并添加依赖：
