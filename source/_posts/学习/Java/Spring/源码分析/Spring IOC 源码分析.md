...
![ApplicationContext继承关系](ApplicationContext继承关系.png)

我们可以使用绿色的 FileSystemXmlApplicationContext 和 AnnotationConfigApplicationContext 这两个类代替 ClassPathXmlApplicationContext：

- FileSystemXmlApplicationContext 基本上和 ClassPathXmlApplicationContext 一样，不同点可以看这篇[文章](https://www.cnblogs.com/ninth/p/6841608.html)。
...
![BeanFactory继承关系](BeanFactory继承关系.png)
...
![DefaultListableBeanFactory继承关系](DefaultListableBeanFactory继承关系.png)
...
> 对于其他的标签，将进入到 delegate.parseCustomElement(element) 这个分支。如我们经常会使用到的 <mvc />、<task />、<context />、<aop />等。
